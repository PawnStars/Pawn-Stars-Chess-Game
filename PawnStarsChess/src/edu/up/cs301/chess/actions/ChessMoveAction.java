...
/**
 * A ChessMoveAction is an action that is a "move" in the game. A piece or
 * pieces are moved on the board and may take opponent pieces.
 * 
...
public class ChessMoveAction extends GameAction {
	...
	// TODO could make it possible to undo a turn with this info and the last
	// position
	 ...
	 */
	public ChessMoveAction(GamePlayer player, ChessPiece whichPiece,
			int[] newPos, ChessPiece takenPiece) {
		super(player);
	...
	public static ChessMoveAction moveTextToAction(ChessGameState state,
			ChessPlayer player, String text) {
		if (text == null || text.equals("(none)")) {
			return null;
		}

		// ensure lower case so capital letters don't affect character
		// arithmetic
		...
		if (text.length() - 4 >= 0) {
			...
			oldY = ChessGameState.BOARD_HEIGHT - text.charAt(text.length() - 3)
					+ 48;
			newX = text.charAt(text.length() - 2) - 97;
			newY = ChessGameState.BOARD_HEIGHT - text.charAt(text.length() - 1)
					+ 48;
		}
		// TODO make sure moves like Bg8 works
		// or gxh4
		// TODO make sure special moves work
		String coords = "x:" + oldX + " y:" + oldY + " newX:" + newX + " newY:"
				+ newY;
		int[] newLoc = new int[] { newY, newX };

		ChessMoveAction move = null;
		if (!ChessGameState.outOfBounds(newLoc)
				&& !ChessGameState.outOfBounds(oldX, oldY)) {
			...
			{
				move = new PawnMove(player, whichPiece, newLoc, takenPiece,
						PawnMove.PROMOTION);
			...
			} else if (enPassant)// en passant
			{
				...
				if (takenPiece != null
						&& takenPiece.getType() == ChessPiece.PAWN) {
					return new PawnMove(player, whichPiece, newLoc,
							takenPiece2, PawnMove.EN_PASSANT);
				} else {
