...
public class ChessGameState extends GameState {
	...
	/*
	 * Keep track of which player is white: True if player 1 is white, False if
	 * player 2 is white
	 */
	private boolean player1IsWhite;

	/*
	 * The index of the two players. Player 1 is at element 0 and player 2 is at
	 * element 1.
	 */
	...
	// Keep track of whether or not this state represents a valid state of the
	// board
	...
	public ChessGameState(boolean player1White) {
		...
		// Give each player pawns of the appropriate color:
	...
	public ChessGameState(ChessGameState orig) {
		...
		pieceMap = new ChessPiece[BOARD_WIDTH][BOARD_HEIGHT];
		player1Pieces = ChessPiece.copyPieceList(pieceMap,
				orig.getPlayer1Pieces());
		player2Pieces = ChessPiece.copyPieceList(pieceMap,
				orig.getPlayer2Pieces());
		...
		for (int i = 0; i < BOARD_WIDTH; ++i) {
			for (int j = 0; j < BOARD_HEIGHT; ++j) {
				pieceMap[i][j] = orig.getPieceMap()[i][j];
			}
		}
	...
	public boolean equals(Object obj) {
		...
		if (canCastle != comp.getCanCastle())
			return false;

		if (player1IsWhite != comp.getPlayer1Color())
			return false;
	...
	public boolean applyMove(GameAction action) {
		...
		if (action instanceof ChessMoveAction) {
			...
			/*
			 * ChessPiece takenPiece = move.getTakenPiece(); if (takenPiece !=
			 * null) { lastCapture = 0;
			 * 
			 * // Search for the taken piece from players' array: for
			 * (ChessPiece p : player1Pieces) { if (p.equals(takenPiece)) { //
			 * kill piece and remove from board p.kill(); int[] loc =
			 * p.getLocation(); pieceMap[loc[0]][loc[1]] = null; } } for
			 * (ChessPiece p : player2Pieces) { if (p.equals(takenPiece)) { //
			 * kill piece and remove from board p.kill(); int[] loc =
			 * p.getLocation(); pieceMap[loc[0]][loc[1]] = null; } } }
			 */
	 ...
	 * 
	 * @param canCastle
	 *            a 2x2 array of booleans that represent whether or not castling
	 *            can occur to the left and right for each player
	 */
	...
	public boolean[][] getPawnMoves(int xLocation, int yLocation,
		...
		if (xLocation - 1 >= 0 && yLocation - 1 >= 0) {
			if (this.pieceMap[yLocation - 1][xLocation - 1] != null
					&& piece.differentColor(this.pieceMap[yLocation - 1][xLocation - 1])) {
	...
	public boolean movePiece(ChessMoveAction act) {
		...
		int newXPos = position[0];
		int newYPos = position[1];

		ChessPiece piece = act.getWhichPiece();
		int oldXPos = piece.getLocation()[0];
		int oldYPos = piece.getLocation()[1];
		...
		if (validMoves[newYPos][newXPos] == true) {
			this.pieceMap[oldXPos][oldYPos] = null;
			this.pieceMap[newXPos][newYPos] = piece;
			piece.move(position);
			return true;
