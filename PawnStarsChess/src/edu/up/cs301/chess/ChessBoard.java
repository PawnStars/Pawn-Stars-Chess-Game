...
public class ChessBoard extends SurfaceView {
	...
	public void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < ChessGameState.BOARD_HEIGHT; i++) {
			for (int j = 0; j < ChessGameState.BOARD_WIDTH; j++) {
				...
				if (selectedLoc != null && selectedLoc[1] == i
						&& selectedLoc[0] == j) {
			...
			// canvas.drawText(""+(ChessGameState.BOARD_HEIGHT-i), 0,
			// (int)((i+0.66)*tileSize[0]), textColor);
		}

		// Draw the chess notation text horizontally
		/*
		 * for(int j=0;j<ChessGameState.BOARD_WIDTH;j++) { // uses arithmetic to
		 * turn 0,1,...,8 to A,B,...,H
		 * canvas.drawText(String.valueOf((char)(65+j)),
		 * (int)((j+0.33)*tileSize[1]), marginSize, textColor); }
		 */
		if (pieceMap != null) {
				...
				for (int j = 0; j < ChessGameState.BOARD_WIDTH; j++) {
					...
					if (piece != null && piece.isAlive()) {
						...
						if (type >= 0 && type < whitePieceStrs.length) {
							if (piece.isWhite()) {
								canvas.drawText(fillTypeStr, x, y,
										whitePieceColor);
		...
		if (this.getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
			...
			if (blackTakenPieces != null && blackTakenPieces.size() > 0) {
				...
				// Goes through the taken black array and draws them on the
				// board
			...
			if (whiteTakenPieces != null && whiteTakenPieces.size() > 0) {
				...
				// Goes through the taken white array and draws them on the
				// board
	...
	public void setPieceMap(ChessPiece[][] pieceMap) {
		...
		if (pieceMap.length != ChessGameState.BOARD_HEIGHT
				|| pieceMap[0].length != ChessGameState.BOARD_WIDTH) {
			return;
		}
		if (this.pieceMap != null) {
			outerloop: for (int i = 0; i < ChessGameState.BOARD_HEIGHT; i++) {
				for (int j = 0; j < ChessGameState.BOARD_WIDTH; j++) {
	...
	/**
	 * Gives the board a new 8x8 array of booleans and draws on the
	 * corresponding true tiles
	 * 
	 * @param 2d array of every piece on the board
	 */
	public void setSelectedTiles(boolean[][] selectedTiles) {
		...
		if (selectedTiles == null) {
			...
			invalidate();
		} else if (selectedTiles.length != ChessGameState.BOARD_HEIGHT
				|| selectedTiles[0].length != ChessGameState.BOARD_WIDTH) {
			return; // error
		} else {
	 ...
	 * 
	 * @param i
	 *            the index of the row down the chess board j the index of the
	 *            columns across the chess board
	 */
	...
	/**
	 * This method adds the white taken pieces to an ArrayList and makes sure
	 * there are no duplicate pieces
	 * 
	 * @param piece
	 *            : The piece that is taken
	 * @param index
	 *            : The index of the piece in the original array
	 */
	...
	/**
	 * This method adds the black taken pieces to an ArrayList and makes sure
	 * there are no duplicate pieces
	 * 
	 * @param piece
	 *            : The piece that is taken
