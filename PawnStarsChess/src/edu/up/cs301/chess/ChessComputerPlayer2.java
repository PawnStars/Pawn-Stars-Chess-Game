...
/**
 * A computer-version of the human player. It generates a list of moves it can
 * make and picks one according to its intelligence level. This computer player
 * does display the game as it is progressing, so if there is no human player on
 * the device, this player will display a GUI that displays the board as the
...
public class ChessComputerPlayer2 extends ChessComputerPlayer1 {
	...
	public void setAsGui(GameMainActivity act) {
		...
		this.player1Score = (TextView) activityForGui
				.findViewById(R.id.player1ScoreTextView);
		this.player2Score = (TextView) activityForGui
				.findViewById(R.id.player2ScoreTextView);

		// Find the board
		board = (ChessBoard) activityForGui
				.findViewById(R.id.gameBoardSurfaceView);
	...
	private void copyAssets() {
		...
		for (String filename : files) {
			if (filename.contains("engine")) {
				...
				try {
					File outFile = new File(activityForGui.getCacheDir(),
							filename);
					if (!outFile.exists()) {
